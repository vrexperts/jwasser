...
<head>
	...
	<title><?php echo prefix." | Comment List";?></title>
	  ...
	  function loadArticle(pageNumber,userid,total_page){
		  ...
		  if(total_page>=pageNumber){
                  ...
                  $.ajax({
                      ...
                      type:'POST',
                      data: "action=infinite_scroll&page_no="+ pageNumber + '&loop_file=loop&userid='+userid, 
		   ...
		   function changestauts(status,id,approved_by){
            $.ajax({url:"ajax-comment-status.php?status="+status+"&id="+id+"&approved_by="+approved_by,success:function(result){
				location.reload();
           }});
        }
...
<div id="tabs-4">
<strong style="font-size:14px;">Comment List</strong>
<div class="pad5"></div>
